...
class ImageMaskDatasetGenerator:
  ...
  def generate(self):
    index = 100000
    label_files = glob.glob(self.labels_dir + "/*.nii")
    ...
    for i in range(l1):
      ...
      self.generate_mask_files(label_files[i], index) 
  ...
  def colorize_mask(self, segmentation):
    # initialize output to zeros
    shp = segmentation.shape
    seg_color = np.zeros((shp[0], shp[1], 3), dtype=np.float32)
    """
     "1": "PZ", peripheral zone
     "2": "TZ"  transitional zone
    """
    #BGR 
    seg_color[np.equal(segmentation, 1)] = (0,0,255)
    seg_color[np.equal(segmentation, 2)] = (0,255,0)
    return seg_color
...
if __name__ == "__main__":
  try:
    images_dir  = "./Task05_Prostate/imagesTr/"
    labels_dir  = "./Task05_Prostate/labelsTr/"

    output_dir  = "./Augmented-Prostate-master/"
